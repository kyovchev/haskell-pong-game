...
renderMenu = 
  ...
  where
    ...
    quit  = translate (-170) (-240) $ color white $ scale 0.2 0.2 $ text "Press ESC to Quit"

renderPause :: GameState -> Picture
renderPause state =
  pictures [ title, pvp, pvai ]
  where
    title = translate (-210)   (50) $ color white $ scale 0.3 0.3 $ text "Game Paused"
    pvp   = translate (-250)  (-80) $ color white $ scale 0.3 0.3 $ text "1 - Resume Game"
    pvai  = translate (-250) (-150) $ color white $ scale 0.3 0.3 $ text "2 - Exit to Menu"
...
renderGameOver state =
  ...
  where
    winner =
      translate (-150) 0 $ color white $ scale 0.3 0.3
       $ text (if score1 state >= pointsToWin then "Player 1 Wins!" else "Player 2 Wins!")
    spaceToMenu =
      translate (-230) (-80) $ color white $ scale 0.3 0.3 $ text "Press SPACE to Menu"
